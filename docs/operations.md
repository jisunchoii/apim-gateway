...
서비스에는 primary 또는 secondary key 하나만 secret manager를 통해 전달합니다. 요청은
`Ocp-Apim-Subscription-Key` header를 사용하며 Keycloak bearer token은 필요하지 않습니다.
서비스별 사용량과 제한 counter는 subscription ID 기준으로 분리되고 Workbook에는 subscription
이름이 사용자 label로 표시됩니다.
